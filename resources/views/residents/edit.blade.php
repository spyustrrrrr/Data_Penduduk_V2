...
<div class="max-w-5xl mx-auto bg-white rounded-2xl shadow-2xl p-10 ring-2 ring-sky-800">
    <div class="mb-8">
        <h1 class="text-4xl font-bold uppercase ">Edit Data Penduduk</h1>
    ...
    <form method="POST" action="{{ route('residents.update', $resident) }}" class="space-y-8 m-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    <label class="{{ $labelClass }}">No. Kartu Keluarga <span class="text-red-600">*</span></label>
                    <input type="text" name="no_kk" required value="{{ old('no_kk', $resident->kk->no_kk ?? '') }}" class="{{ $inputKK }}" placeholder="16 digit No. KK">
                ...
                <div>
                    <label class="{{ $labelClass }}">Alamat <span class="text-red-600">*</span></label>
                    <input type="text" name="alamat" required value="{{ old('alamat', $resident->alamat ?? $resident->kk->alamat ?? '') }}" class="{{ $inputKK }}" placeholder="Alamat lengkap">
                    @error('alamat') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="{{ $labelClass }}">RT</label>
                    <input type="text" name="rt" value="{{ old('rt', $resident->kk->rt ?? '') }}" class="{{ $inputKK }}" placeholder="001" maxlength="3">
                </div>

                <div>
                    <label class="{{ $labelClass }}">RW</label>
                    <input type="text" name="rw" value="{{ old('rw', $resident->kk->rw ?? '') }}" class="{{ $inputKK }}" placeholder="001" maxlength="3">
                </div>

                <div>
                    <label class="{{ $labelClass }}">Kelurahan/Desa</label>
                    <input type="text" name="kelurahan" value="{{ old('kelurahan', $resident->kk->kelurahan ?? '') }}" class="{{ $inputKK }}" placeholder="Nama Kelurahan">
                </div>

                <div>
                    <label class="{{ $labelClass }}">Kecamatan</label>
                    <input type="text" name="kecamatan" value="{{ old('kecamatan', $resident->kk->kecamatan ?? '') }}" class="{{ $inputKK }}" placeholder="Nama Kecamatan">
                </div>
            </div>
        </div>
        ...
        {{-- Data Pribadi --}}
        <div>
            <h4 class="text-lg font-semibold text-gray-900 mb-4 pb-3 border-b-2 border-amber-600">
                <i class="fas fa-user text-amber-600 mr-2"></i>Data Pribadi
            </h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="{{ $labelClass }}">NIK <span class="text-red-600">*</span></label>
                    <input type="text" name="nik" required value="{{ old('nik', $resident->nik) }}" class="{{ $inputDP }}" placeholder="16 digit NIK">
                    @error('nik') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="{{ $labelClass }}">Nama Lengkap <span class="text-red-600">*</span></label>
                    <input type="text" name="nama" required value="{{ old('nama', $resident->nama) }}" class="{{ $inputDP }}" placeholder="Nama lengkap sesuai KTP">
                    @error('nama') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="{{ $labelClass }}">Jenis Kelamin <span class="text-red-600">*</span></label>
                    <select name="jenis_kelamin" required class="{{ $inputDP }}">
                ...
                <div>
                    <label class="{{ $labelClass }}">Tempat Lahir <span class="text-red-600">*</span></label>
                    <input type="text" name="tempat_lahir" required value="{{ old('tempat_lahir', $resident->tempat_lahir) }}" class="{{ $inputDP }}" placeholder="Kota/Kabupaten">
                    @error('tempat_lahir') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="{{ $labelClass }}">Tanggal Lahir <span class="text-red-600">*</span></label>
                    <input type="date" name="tanggal_lahir" required value="{{ old('tanggal_lahir', optional($resident->tanggal_lahir)->format('Y-m-d')) }}" class="{{ $inputDP }}">
                    @error('tanggal_lahir') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="{{ $labelClass }}">Agama <span class="text-red-600">*</span></label>
                ...
                <div>
                    <label class="{{ $labelClass }}">Status Perkawinan <span class="text-red-600">*</span></label>
        ...
        <div class="col-span-2">
            <label class="{{ $labelClass }}">Foto KTP (Opsional)</label>
            <div class="{{ $inputDP }} flex items-center gap-5">
                {{-- Preview Foto Lama --}}
                @if($resident->foto_ktp)
                <div class="w-32  me-6">
                    <p class="text-gray-800 font-semibold" > Foto Saat Ini </p>
                    <img src="{{ asset('storage/' . $resident->foto_ktp) }}" alt="Foto KTP"
                    class="w-full rounded-xl shadow-sm" >
                </div>
                @endif
                {{-- Upload Foto Baru --}}
                <div class="flex-grow">
                    <label for="foto_baru" class="text-gray-800 font-semibold">Tambahkan Foto Baru</label>
                    <input id="foto_baru" type="file" name="foto_ktp"
                    class="w-full text-sm text-gray-700 file:mr-4 file:py-2.5 file:px-5
                    file:rounded-xl file:border-0 file:bg-amber-600 file:text-white
                    file:font-semibold hover:file:bg-amber-600 transition bg-white
                    border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-600">
                    <p class="text-xs text-gray-500 mt-2"> Format: JPG, JPEG, PNG • Maksimal 2MB </p>
                     @error('foto_ktp') <p class="text-red-600 text-sm mt-2 font-medium"> {{ $message }}
                        </p>
                    @enderror
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="status_merokok" class="{{ $inputDK }}">
                    <option value="">Pilih</option>
            ...
            <div>
                ...
                <input type="text" name="riwayat_penyakit" class="{{ $inputDK }}" placeholder="Jika ada, sebutkan riwayat penyakit..." value="{{ old('riwayat_penyakit', $resident->riwayat_penyakit) }}">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="no_telepon" value="{{ old('no_telepon', $resident->no_telepon) }}" class="{{ $inputDKL }}" placeholder="Nomor telepon">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Email</label>
                <input type="email" name="email" value="{{ old('email', $resident->email) }}" class="{{ $inputDKL }}" placeholder="Alamat email">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Pekerjaan</label>
                <input type="text" name="pekerjaan" value="{{ old('pekerjaan', $resident->pekerjaan) }}" class="{{ $inputDKL }}" placeholder="Profesi/Pekerjaan">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Ayah</label>
                <input type="text" name="nama_ayah" value="{{ old('nama_ayah', $resident->nama_ayah) }}" class="{{ $inputDKL }}" placeholder="Nama ayah">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Ibu</label>
                <input type="text" name="nama_ibu" value="{{ old('nama_ibu', $resident->nama_ibu) }}" class="{{ $inputDKL }}" placeholder="Nama ibu">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">BPJS Ketenagakerjaan</label>
                <select name="bpjs_ketenagakerjaan" class="{{ $inputDKL }}">
                    <option value="">Pilih</option>
            ...
            <div>
                ...
                <select name="tambah_anak" class="{{ $inputDKL }}">
                    <option value="">Pilih</option>
            ...
            <div>
                ...
                <input type="number" name="jumlah_anak" value="{{ old('jumlah_anak', $resident->jumlah_anak) }}" min="0" class="{{ $inputDKL }}" placeholder="Jumlah anak">
            </div>
            <div>
        ...
        <div class="flex gap-4 pt-6 border-t-2 border-gray-300">
            <button type="submit"
            ...
            <i class="fas fa-save"></i> Perbarui Data </button>
            <a href="{{ route('residents.index') }}"
            class="inline-flex items-center gap-2 bg-red-500 hover:bg-red-600
            text-gray-50 px-6 py-3 rounded-lg transition font-semibold">
            <i class="fas fa-times"></i> Batal </a> </div> </form> </div>
    </form>
</div>
